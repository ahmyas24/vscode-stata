 ...
 * 4. Create Python 3.13 virtual environment with uv
...
function promptUvInstallation() {
	...
	if (process.platform === "win32") {
		...
		console.log("\nFor macOS/Linux:");
		console.log(
			"Option 1 (may require sudo): curl -LsSf https://astral.sh/uv/install.sh | sudo sh"
		);
		console.log(
			"Option 2 (user install): curl -LsSf https://astral.sh/uv/install.sh | sh"
		);
	}
	...
	console.log(
		'2. Verify UV is installed by running "uv --version" in a terminal'
	);
...
function isUvInstalled() {
	...
	if (fs.existsSync(uvPathFile)) {
		...
		if ((isWin32 && !pathHasBackslash) || (!isWin32 && pathHasBackslash)) {
			console.log(
				`Warning: Saved UV path is not compatible with current platform (${process.platform}). Ignoring saved path.`
			);
			try {
			...
			console.log(
				`Saved UV path doesn't exist or is not executable: ${savedPath}`
			);
			try {
				fs.unlinkSync(uvPathFile);
	...
	try {
		const version = execSync("uv --version", { stdio: "pipe" })
			.toString()
			.trim();
		...
		try {
			if (process.platform === "win32") {
				const pathOutput = execSync("where uv", { stdio: "pipe" })
					.toString()
					.trim()
					.split("\n")[0];
				uvPath = pathOutput;
			} else {
				const pathOutput = execSync("which uv", { stdio: "pipe" })
					.toString()
					.trim();
				uvPath = pathOutput;
			}
		...
		for (const uvPath of commonPaths) {
			...
			if (fs.existsSync(uvPath) && isExecutable(uvPath)) {
				...
				// Verify it works
				try {
					const version = execSync(`"${uvPath}" --version`, { stdio: "pipe" })
						.toString()
						.trim();
					...
					console.log(
						`Found UV at ${uvPath} but verification failed: ${verifyError.message}`
					);
				}
			}
		}
...
async function installUv() {
	...
	try {
		...
		if (process.platform === "win32") {
			...
			try {
				fs.mkdirSync(path.join(os.homedir(), ".local", "bin"), {
					recursive: true,
				});
			} catch (err) {
				console.log(
					`Note: Could not ensure ~/.local/bin exists: ${err.message}`
				);
			}
			...
			if (uvInfo.installed) {
				...
				if (process.platform === "win32") {
					...
					try {
						...
						await execPromise(
							`cd "${tempDir}" && powershell -Command "& {Expand-Archive -Path uv.zip -DestinationPath .}""`
						);
						...
						fs.copyFileSync(
							path.join(tempDir, "uv.exe"),
							path.join(userBinDir, "uv.exe")
						);
						...
						console.error(
							`Alternative installation failed: ${altError.message}`
						);
					...
					if (platform !== "unknown") {
						...
						try {
							...
							await execPromise(
								`cd "${tempDir}" && curl -L ${downloadUrl} -o uv.tar.gz`
							);
...
async function setupPythonWithUv() {
	...
	try {
		...
		const createVenvCmd = `${uvCmd} venv "${venvPath}" --python 3.13`;
		...
		try {
			...
			const installCmd = `${uvCmd} pip install --python "${pythonPath}" ipykernel nbstata`;
			...
			if (fs.existsSync(pythonPath)) {
				...
				console.error(
					`Python executable not found at expected path: ${pythonPath}`
				);
				promptUvInstallation();
				return false;
			}
			...
			console.error(
				`Error installing Python dependencies with uv: ${error.message}`
			);
			promptUvInstallation();
			return false;
		}
...
async function main() {
	...
	if (process.env.NODE_ENV === "production" && fs.existsSync(uvPathFile)) {
		...
		try {
			...
			console.warn(
				`Failed to remove .uv-path in production mode: ${error.message}`
			);
		}
	}
